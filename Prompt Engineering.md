...
Chain of Thought (CoT) prompting is a technique for improving the reasoning capabilities by **generating intermediate reasoning steps**.

Pros:
1. It’s low-effort, effective and works well with off-the-shelf LLMs (no need to finetune).
2. Better interpretability, as it can learn from the LLM’s responses and see the reasoning steps that were followed. And good for identifying malfunction.
3. CoT appears to improve robustness when moving between different LLM versions, so the performance of the prompt should drift less between different LLMs than without CoT.

Cons:
It leads more output tokens, which means predictions cost more money and take longer.
...
### Self-consistency
LLMs' ability to reason is often seen as a limitation that cannot be overcome solely by increasing model size. As discussed in the CoT prompting section, the model can be prompted to generate reasoning steps like a human solving a problem. However CoT uses a simple ‘greedy decoding’ strategy, limiting its effectiveness. 

Self-consistency combines sampling and majority voting to generate diverse reasoning paths and select the most consistent answer. Self-consistency improves the accuracy and coherence of responses generated by LLMs. And it gives a pseudo-probability likelihood of an answer being correct, but obviously has high costs.
